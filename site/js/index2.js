...
$(document).ready(function(){
  ...
  //批量预载视频
	function preload(list,onLoaded,onCompleted,el) {
    let p = []
  ...
  function initPlayList(el) {
    ...
    var index = 0;
    video.src = JSON.parse(video.dataset.playlist)[index];
    $(video).on("ended", function () {
      index = index === JSON.parse(video.dataset.playlist).length - 1 ? 0 : index + 1;
      video.src = JSON.parse(video.dataset.playlist)[index];
      video.play();
    })
  }
  ...
  function initHome() {
    API.getSiteInfo({id:siteId},function(res){      
      ...
      var videos = {
        siteVideo:res.result.siteVideo
      }
      var videoX1 = {
      ...
      var version = "20231207";
      banners.siteBanner = banners.siteBanner.map(i=>i+"?v"+version);
      bannersX1.siteBanner = bannersX1.siteBanner.map(i=>i+"?v"+version);
      bannersX2.siteBanner = bannersX2.siteBanner.map(i=>i+"?v"+version);
      videos.siteVideo = videos.siteVideo.map(i=>i+"?v"+version);
      videoX1.siteVideo = videoX1.siteVideo.map(i=>i+"?v"+version);
      videoX2.siteVideo = videoX2.siteVideo.map(i=>i+"?v"+version);
      ...
      if(screenType == 'vertical'){
        Template('tpl-video', videos);
        initPlayList('.video-y1 video');
        preload([videos.siteVideo[0]], function(r){
          console.log(r)
        },function(all){
          var list = JSON.parse(all[0].$el[0].dataset.playlist);
          list[all[0].index] = all[0].blob;
          all[0].$el[0].dataset.playlist = JSON.stringify(list);
          all[0].$el[0].src =  all[0].blob;
          preload(videos.siteVideo, function(r){
            console.log(r)
            var list = JSON.parse(r.$el[0].dataset.playlist);
            list[r.index] = r.blob;
            r.$el[0].dataset.playlist = JSON.stringify(list);
          },function(all){
            console.log(all)
          },'.video-y1 video');
        },'.video-y1 video');
        ...
        initPlayList('.video-x1 video');
        initPlayList('.video-x2 video');
        preload(videoX1.siteVideo, function(r){
          console.log(r)
          var list = JSON.parse(r.$el[0].dataset.playlist);
          list[r.index] = r.blob;
          r.$el[0].dataset.playlist = JSON.stringify(list);
        ...
        preload(videoX2.siteVideo, function(r){
          console.log(r)
          var list = JSON.parse(r.$el[0].dataset.playlist);
          list[r.index] = r.blob;
          r.$el[0].dataset.playlist = JSON.stringify(list);
