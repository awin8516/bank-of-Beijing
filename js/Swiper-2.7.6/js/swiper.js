...
var Swiper = function (selector, params) {
    ...
    for (var i = _this.container.childNodes.length - 1; i >= 0; i--) {
        if (_this.container.childNodes[i].className) {
            ...
            for (var j = 0; j < _wrapperClasses.length; j++) {
				...
				/***************************
				 * F start
				 * 增加一个遮罩容器 params.maskClass，方便特殊布局，左右切换在面板外
				 */
				if (_wrapperClasses[j] === params.maskClass) {
					for (var m = _this.container.childNodes[i].childNodes.length - 1; m >= 0; m--) {
						if (_this.container.childNodes[i].childNodes[m].className && _this.container.childNodes[i].childNodes[m].className.indexOf(params.wrapperClass) != -1 ) {
							wrapper = _this.container.childNodes[i].childNodes[m];
						}
					}
                }
				/*****
	...
	/* 修复pagination包含子元素时点击无效的BUG，冒泡失效*/
    ...
    function paginationClick(e) {
        ...
        var pagers = _this.paginationButtons;
        for (var i = 0; i < pagers.length; i++) {
            if (target === pagers[i] || isParent(target, pagers[i]) ) index = i;
        }
        if (params.autoplay) _this.stopAutoplay(true);
        _this.swipeTo(index);
    }
...
Swiper.prototype = {
	...
	getTranslate : function (_el) {
        ...
        if (this.support.transforms && this.params.useCSS3Transforms) {
			...
			//Latest Chrome and webkits Fix
			if (window.WebKitCSSMatrix)
				curTransform.x = transformMatrix.m41,
				curTransform.y = transformMatrix.m42;
			//Crazy IE10 Matrix
			else if (matrix.length === 16)
				curTransform.x = parseFloat(matrix[12]),
				curTransform.y = parseFloat(matrix[13]);
			//Normal Browsers
			else
				curTransform.x = parseFloat(matrix[4]),
				curTransform = parseFloat(matrix[5]);
           
        }
        else {
            curTransform.x = parseFloat(el.style.left, 10) || 0;
            curTransform.y = parseFloat(el.style.top, 10) || 0;
        }
        return curTransform || 0;
    },
